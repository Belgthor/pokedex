...
@Component({
  ...
  templateUrl: './trainer-xxl.html',
  styleUrl: './trainer-xxl.css'
})
export class TrainerXxl {
  ...
  totalCheck(myArray:any){
    const totalItems = myArray.reduce((count:any, Arrayobj:any) => {
      let found = Arrayobj.trainer.find((obj:any) => obj.name == this.trainer && obj.xxl === true)
      if(found){
        return count + 1;
      }
      return count;
  ...
  updateXXL(mon:any){
    ...
    if(found){
      found.xxl = found.xxl ? false : true;
      this.pokemonService.updatePokemon(mon._id,found.name,{xxl:found.xxl}).subscribe({
        next: (data: any) => {
      ...
      this.pokemonService.updatePokemon(mon._id,this.trainer,{xxl:true}).subscribe({
        next: (data: any) => {
          this.socketService.updatePokemon(mon._id);
        },
        error: (e) => console.error(e)
